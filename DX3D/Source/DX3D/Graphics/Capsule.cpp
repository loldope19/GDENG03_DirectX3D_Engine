...
namespace dx3d
{
	...
	Capsule::Capsule(const GraphicsResourceDesc& gDesc) : GameObject(gDesc)
	{
		m_stride = sizeof(CapsuleVertex);
		m_offset = 0;

		std::vector<CapsuleVertex> vertices;
		std::vector<UINT> indices;

		// --- Capsule Generation Logic ---
		const int latitudeBands = 30;
		const int longitudeBands = 30;
		const float radius = 0.5f;
		const float cylinderHeight = 1.0f;
		const float halfHeight = cylinderHeight / 2.0f;
		...
		for (int lat = 0; lat <= latitudeBands; ++lat)
		{
			...
			for (int lon = 0; lon <= longitudeBands; ++lon)
			{
				...
				if (vs.position.y > 0)
					vs.position.y += halfHeight;
				else
					vs.position.y -= halfHeight;

				vs.color.x = (vs.position.x + radius) / (2 * radius);
				vs.color.y = (vs.position.y + radius + halfHeight) / (2 * radius + cylinderHeight);
		...
		bufferDesc.ByteWidth = static_cast<UINT>(sizeof(CapsuleVertex) * vertices.size());
		bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
		D3D11_SUBRESOURCE_DATA initData = { vertices.data() };
		DX3DGraphicsLogThrowOnFail(m_device.CreateBuffer(&bufferDesc, &initData, &m_vertexBuffer), "Failed to create capsule vertex buffer");
		...
		DX3DGraphicsLogThrowOnFail(m_device.CreateBuffer(&indexBufferDesc, &indexInitData, &m_indexBuffer), "Failed to create capsule index buffer");
		...
		{
			DX3DLogThrowError("Failed to load/compile vertex shader for Capsule.");
		}
		...
		{
			DX3DLogThrowError("Failed to load/compile pixel shader for Capsule.");
		}
